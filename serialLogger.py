...
serial_logger = logging.getLogger('main_logger.serial')
serial_logger.setLevel(logging.INFO)
serial_logger.propagate = True
...
def create_config_shelve():
    ...
    serial_logger.info('Configuration complete')
...
def generate_serial_number(serial_type):
    ...
    with shelve.open('serial_config', writeback=True) as config:
        ...
        serial_logger.info('new serial number created: {}'.format(new_serial))
    return new_serial
